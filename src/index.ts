...
import type DDS_FILE_LAYOUT from "./types/DDS_FILE_LAYOUT.js";

export default function readDDS(byteArray: ArrayBuffer, calculatePitch = true, blockSize?: number){
    ...
    const ddsFileLayout: DDS_FILE_LAYOUT = {
        dwMagic: metaData[i++],
        ...
        header: {
            ...
            */
            ...
            dwReserved1: metaData.slice( i, (i = i+11) ), // indexes 8 - 18      
    ...
    if(
        ddsFileLayout.header.ddspf.dwFlags.DDPF_FOURCC && 
    ...
    if(ddsFileLayout.header.dwFlags.DDSD_LINEARSIZE)
        ...
        const pitch = manuallyCalculatePitch(
            blockSize,
            ddsFileLayout.header.dwWidth,
            ddsFileLayout.header.ddspf.dwRGBBitCount,
            ddsFileLayout.header.ddspf.dwFourCC,
            ddsFileLayout.header10?.dxgiFormat
        );
        if(pitch <= 0){
            // TODO: test if the message works
            const message = 
                "Pitch could not be manually calculated.\n" +
